    ...
    dbc.Collapse(
        dbc.Card([   
            dbc.CardBody([
                ...
                dbc.Input(id="inpt-cut-name", placeholder="Nome do corte - tete", 
                                type="text"),
        ...
        id="collapse", is_open=True, style={"margin-top": "25px", 
        "margin-bottom": "25px"}),
...
def update_btn_start(n_clicks,value):
...
def update_btn_end(n_clicks,value):
...
@app.callback(Output('dd-cut-scenes', 'options'),
              ...
              [State('btn-set-start', 'children'), State('btn-set-end', 'children'), State('inpt-cut-name', 'value'), State('rd-cut-kind', 'value'), State('dd-cut-scenes', 'value')])
